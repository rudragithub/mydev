...
{ 
	...
	document.getElementById('clientAddtable').innerHTML ="<tbody><tr><th style='height: 30px;' class='colum'>Client Name</th><th style='height: 30px;' class='colum'>BU Head</th><th style='height: 30px;' class='colum'>Target Date</th><th style='height: 30px;' class='colum'>Shared Date</th><th style='height: 30px;' class='colum'>Add / Remove</th></tr>";	
	...
	if (window.XMLHttpRequest) {    
	 ...
	 xmlhttp.onreadystatechange=function() {
        ...
        if (xmlhttp.readyState==4 && xmlhttp.status==200) {		
			...
			for(var i=1;i<=rows;i++)
			{
				...
				document.clientUpdateForm["data2_"+i].value =data.split("||")[1];
				document.clientUpdateForm["targetDay_"+i].value =String(data.split("||")[2]).split("-")[2];
				document.clientUpdateForm["targetMonth_"+i].value =String(data.split("||")[2]).split("-")[1];
				document.clientUpdateForm["targetYear_"+i].value =String(data.split("||")[2]).split("-")[0];
				document.clientUpdateForm["scheduleDay_"+i].value =String(data.split("||")[3]).split("-")[2];
				document.clientUpdateForm["scheduleMonth_"+i].value =String(data.split("||")[3]).split("-")[1];
				document.clientUpdateForm["scheduleYear_"+i].value =String(data.split("||")[3]).split("-")[0];
	 ...
	 var queryString = "?ps_no=" + document.getElementById('reg_psno').value ;
	
	xmlhttp.open("POST","clientPopupdata.php" +queryString,true);
    xmlhttp.send();	
...
function createClientRowData(id)
{
	...
	//var clientList = ['','Samsung','cisco','IDC'];
	...
	sel.setAttribute('name','data1_'+id)
	for(var i=0;i<clientList.length;i++)
	{
		var o = document.createElement('option');
		o.setAttribute('value',clientList[i]);
		o.innerHTML=clientList[i];
		sel.appendChild(o);
	}
